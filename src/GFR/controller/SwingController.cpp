...
chassis::DiffChassisCommand SwingController::get_angular_command(
    ...
    angular_error = gfr::norm_delta(angular_error);
    bool chainedExecutable = false;
    if (fabs(angular_error) < gfr::to_radians(5)) {
        if (thru) {
    ...
    if (!turn_overshoot) {
        if (direction == gfr::AngularDirection::COUNTERCLOCKWISE &&
            angular_error < 0) {
            angular_error += 2 * M_PI;
        } else if (direction == gfr::AngularDirection::CLOCKWISE &&
                   angular_error > 0) {
            angular_error -= 2 * M_PI;
        }
    }
